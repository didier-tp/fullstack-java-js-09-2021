...
//et dépendances springfox-swagger2 , springfox-swagger-ui dans pom.xml
	...
	public Docket api() {
		return new Docket(DocumentationType.SWAGGER_2).select().apis(/* RequestHandlerSelectors.any() */
				RequestHandlerSelectors.basePackage("tp.appliJee.rest")).paths(PathSelectors.any()).build()
				.apiInfo(apiInfo());
	}

	private ApiInfo apiInfo() {
		return new ApiInfoBuilder().title("My BANK REST APIs").version("1.0.0").build();
	}
}
